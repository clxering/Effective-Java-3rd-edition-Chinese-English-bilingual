...
如果初始化了包含改进版本的isRomanNumeral方法的类，但从未调用该方法，那么初始化字段ROMAN是不必要的。因此，可以在第一次调用isRomanNumeral方法时，延迟初始化字段(项目83)，以此代替类加载时初始化，但不建议这样做。通常情况下，延迟初始化会使实现复杂化，而没有明显的性能改进（[Item-67](https://github.com/clxering/Effective-Java-3rd-edition-Chinese-English-bilingual/blob/master/Chapter-9-Item-67-Optimize-judiciously.md)）。
