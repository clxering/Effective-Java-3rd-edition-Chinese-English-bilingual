...
位字段表示还允许你使用位算术高效地执行集合操作，如并集和交集。但是位字段具有 int 枚举常量等所有缺点。当位字段被打印为数字时，它比简单的 int 枚举常量更难解释。没有一种简单的方法可以遍历由位字段表示的所有元素。最后，你必须预测在编写 API 时需要的最大位数，并相应地为位字段（通常是 int 或 long）选择一种类型。一旦选择了一种类型，在不更改 API 的情况下，不能超过它的宽度（32 或 64 位）。
