...
自 Java 5 以来，泛型一直是 Java 语言的一部分。在泛型之前，必须对每个从集合中读取的对象进行强制转换。如果有人不小心插入了错误类型的对象，强制类型转换可能在运行时失败。对于泛型，你可以告知编译器在每个集合中允许哪些类型的对象。编译器会自动为你进行插入转换，如果你试图插入类型错误的对象，编译器会在编译时告诉你。这就产生了既安全又清晰的程序，但是这些好处不仅仅局限于集合，而且是有代价的。这一章告诉你如何最大限度地提高效益和减少不良因素。
