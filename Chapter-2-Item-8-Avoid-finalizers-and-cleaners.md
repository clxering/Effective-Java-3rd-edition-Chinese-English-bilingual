...
终结器和清除器的一个缺点是不能保证它们会被立即执行[JLS, 12.6]。（因为）从对象变得不可访问到结束器或清理器运行的时间是不确定的。这意味着您永远不应该在终结器或清理器中执行任何时间关键的操作。例如，依赖终结器或清理器关闭文件是一个严重错误，因为打开的文件描述符是有限的资源。如果由于系统在运行终结器或清除器时的延迟导致许多文件处于打开状态，程序可能会失败，因为它不能再打开文件。
